...
**Tests**: Include pytest tasks for every public-facing function and MCP tool.
Add tests for internal code when its risk or complexity warrants them.
...
- [ ] T010 Implement bounded API retry policy with `Retry-After`, increasing backoff, jitter, safe retry classification, and exhaustion handling in src/services/retry.py
...
### Tests for User Story 1 (REQUIRED for public functions and MCP tools) ⚠️
...
### Tests for User Story 2 (REQUIRED for public functions and MCP tools) ⚠️
...
### Tests for User Story 3 (REQUIRED for public functions and MCP tools) ⚠️
...
- [ ] TXXX [P] Additional risk-based internal unit tests in tests/unit/
- [ ] TXXX Security hardening
- [ ] TXXX Run quickstart.md validation
- [ ] TXXX Independent QA sub-agent validates required tests and runs `uv run ruff check .`, `uv run ruff format --check .`, and `uv run pytest`
...
- Required public-surface tests MUST be written before or alongside implementation;
  risk-based internal tests MUST be added when warranted
