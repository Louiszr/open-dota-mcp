...
5. **Given** no other optional filters, **When** a report is requested, **Then** each returned eligible game identifies the match, tournament, patch label, date, analyzed team, opponent ID and name, side, team-relative win/loss, first- or second-ban order when known, and duration.
...
- A version expression is malformed, longer than 64 characters, or exceeds the 50-millisecond full-match evaluation timeout: reject it with `invalid_version_expression`, advise simplifying it, and do not begin match-detail retrieval.
...
- A match-detail request fails or returns a malformed record: the match remains in the examined quota, counts as unparsed, does not become a public match outcome, and does not prevent otherwise usable matches from being returned.
...
- **FR-018**: The capability MUST reject a malformed version expression before retrieving match details. Every full-string evaluation MUST have a 50-millisecond timeout. If evaluation exceeds that timeout, the request MUST fail with the same structured `invalid_version_expression` error used for malformed expressions, advise the caller to simplify the expression, and MUST NOT expose regular-expression engine diagnostics. The public description MUST explain full-string matching, the 64-character limit, and the evaluation timeout.
...
- **FR-021b**: Parse coverage MUST be limited to aggregate parsed and unparsed counts for the complete lookback. For coverage purposes, a match is parsed only when a usable parsed detail record is obtained. A missing, malformed, or failed detail record MUST count as unparsed, so parsed count plus unparsed count MUST always equal examined count. The public response MUST NOT include per-filter-stage coverage, filter evaluations, or explanations of why individual matches are unparsed or excluded.
- **FR-021c**: Unparsed, filtered-out, malformed, and failed-detail matches MUST NOT be expanded into public match outcomes. A malformed or failed-detail match MUST count as unparsed; otherwise these matches contribute only to the compact lookback summary. Optional evidence MUST never be fabricated.
...
- **SC-006**: For complete parsed-match fixtures, 100% of requested core and optional fields, including opponent ID, agree with source team identity, draft order, player assignments, lane time series, structure events, objective events, and analyzed-team perspective at the defined checkpoints; no response contains a team tag, league ID, patch ID, hero ID, player account ID, source/provenance field, or filter evaluation.
...
- **SC-007a**: For every fixture set, including missing, malformed, and failed-detail records, parsed count plus unparsed count equals the examined lookback count, with each unusable detail counted as unparsed and no per-filter-stage coverage breakdown.
