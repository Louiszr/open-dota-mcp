...
3. **Given** a candidate combination of emblems, quality tiers, traits, and titles, **When** an agent combines that configuration with a historical map's raw statistics and the scoring reference, **Then** it can calculate the projected score that configuration would have produced, while preserving unknown effects as uncertainty and never representing the candidate configuration as historical match data.
...
- **FR-002**: Name matching MUST normalize both the query and catalog names by applying Unicode NFKC normalization, Unicode case folding, replacing each contiguous run of punctuation or whitespace with a single ASCII space, and trimming leading or trailing spaces. A query whose normalized value is empty MUST be rejected. The capability MUST prefer a unique normalized exact professional-name match; otherwise it MUST return at most 10 concise candidates ordered by normalized professional name and then stable account ID, and require explicit selection.
- **FR-003**: The capability MUST accept a requested match count from 1 through 100, defaulting to 20. It MUST apply the count after all filters and return maps newest first. It MUST examine no more than 500 player-history records, in pages of at most 100 records, and hydrate no more than 200 unique match-detail records per request. These fixed internal safety limits are not caller-configurable. The capability MUST stop earlier when the requested result count is established, source history is exhausted, caller cancellation occurs, or the caller deadline is reached. When a safety limit prevents exhaustive evaluation, the response MUST identify the reached limit, report examined and hydrated counts, set `truncated` to true, and warn that additional eligible maps may exist. Because the caller requests a finite bounded collection, public pagination is not required for this capability.
...
- **FR-021**: Historical map responses MUST contain observed match evidence and pre-modifier raw emblem scores only. Emblem quality tiers, traits, titles, and banner configurations MUST NOT be modeled as properties of historical matches. The scoring reference MUST provide the formulas, modifier rules, application order, scope, prerequisites, and evidence status needed for a client to apply a candidate configuration retrospectively. Any resulting post-modifier value MUST be treated as a counterfactual projection for that candidate configuration, not an observed historical score.
...
- **Candidate Fantasy Configuration**: An agent-selected combination of emblems, quality tiers, traits, titles, and banner choices applied retrospectively to historical evidence; it is not a property of any historical match.
- **Projected Fantasy Score**: A counterfactual result calculated by combining historical raw evidence with one candidate fantasy configuration and the versioned scoring reference.
...
- **SC-007**: Across a fixed corpus of 20 scripted agent-evaluation cases covering at least two players, two emblem combinations, known and unknown series IDs, unavailable statistics, and known and unknown modifiers, at least 18 cases MUST produce the expected comparison using only the player capability and scoring reference, without a live rules search or clarification about field meaning. A case passes only when the selected evidence, computed raw scores, series treatment, and stated uncertainty match its expected result.
...
- Quality tiers, traits, titles, and banner choices describe a candidate fantasy configuration, not an intrinsic property of a historical match. Agents apply candidate configurations retrospectively to historical raw evidence; projected post-modifier values are counterfactual calculations rather than observed historical scores.
