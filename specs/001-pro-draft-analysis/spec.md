...
- A request contains duplicate match IDs: return each distinct match once, in the position of its first occurrence, and report that duplicates were omitted.
...
- **FR-005**: The draft capability MUST accept an ordered collection of 1 to 10 match IDs, validate each ID, de-duplicate repeated IDs, and report results in first-occurrence request order.
...
- **FR-012**: The tournament capability MUST return eligible matches newest first. Its slim match record MUST include match ID, UTC start time, league ID/name, Radiant and Dire team IDs/names, winner, final score when available, and data-completeness warnings.
...
- **FR-019**: All tools MUST preserve authoritative numeric identifiers alongside labels, use UTC ISO 8601 timestamps, distinguish absent data from false/zero values, and attach warnings to the affected record rather than only at response level.
