...
4. **Generate tasks.md**: Read the tasks template from TASKS_TEMPLATE (from the JSON output above) and use it as structure. If TASKS_TEMPLATE is empty, fall back to `.specify/templates/tasks-template.md`. Fill with:
   ...
   - Each phase includes: story goal, independent test criteria, constitution-required
     public-surface tests, any warranted internal tests, and implementation tasks
...
**Tests follow the constitution**: Generate pytest tasks for every public-facing
function and MCP tool. Generate internal-code test tasks when risk or complexity
warrants them, or when the feature specification requests them.

**API resilience follows the constitution**: For MCP tools that call APIs, generate
implementation and deterministic test tasks for retryable-error classification,
...
1. **From User Stories (spec.md)** - PRIMARY ORGANIZATION:
   ...
   - Map all related components to their story:
     ...
     - Required public-function and MCP-tool tests specific to that story
     - Risk-based internal tests when warranted
   - Mark story dependencies (most stories should be independent)

2. **From Contracts**:
   - Map each interface contract → to the user story it serves
   - Each public interface contract → contract test task [P] before or alongside
